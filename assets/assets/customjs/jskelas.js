...
$(document).ready(function () {
	...
	$(
		"#loading-simpan, #loading-ubah, #loading-hapus, #pesan-error, #btn-reset"
	).hide();

	// Fungsi ini akan dipanggil ketika tombol edit diklik
	$("#view").on("click", ".btn-form-ubah", function () {
		// Ketika tombol dengan class btn-form-ubah pada div view di klik
		id_kelas = $(this).data("id_kelas"); // Set variabel id dengan id yang kita set pada atribut data-id pada tag button edit
		...
		var kelas = tr.find(".kelas-value").val(); // Ambil nis dari input type hidden

		$("#kelas").val(kelas); // Set value dari textbox nis yang ada di form
	});

	// Fungsi ini akan dipanggil ketika tombol hapus diklik
	$("#view").on("click", ".btn-alert-hapus", function () {
		// Ketika tombol dengan class btn-alert-hapus pada div view di klik
		id_kelas = $(this).data("id_kelas"); // Set variabel id dengan id yang kita set pada atribut data-id pada tag button hapus
	});
	...
	$("#btn-simpan").click(function () {
		...
		$.ajax({
			url: base_url + "Master/saveKelas", // URL tujuan
	...
	$("#btn-ubah").click(function () {
		...
		$.ajax({
			url: base_url + "Master/editKelas/" + id_kelas, // URL tujuan
			type: "POST", // Tentukan type nya POST atau GET
			data: $("#form-modal form").serialize(), // Ambil semua data yang ada didalam tag form
			dataType: "json",
			beforeSend: function (e) {
	...
	$("#btn-hapus").click(function () {
		...
		$.ajax({
			url: base_url + "Master/deleteKelas/" + id_kelas, // URL tujuan
